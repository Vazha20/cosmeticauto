...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      setIsFetching(true);
      try {
        const res = await axios.get('https://chupstore.onrender.com/api/users/profile', { headers: { Authorization: `Bearer ${token}` } });
        if (res.data) {
          setForm({
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await axios.put('https://chupstore.onrender.com/api/users/profile', { ...form }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleProductSubmit = async (values: any) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("https://chupstore.onrender.com/api/products", {
        method: "POST",
        body: formData,
      });
