...
export default function ProductsPage() {
  ...
  useEffect(() => {
    fetch("https://chupstore.onrender.com/api/products")
      .then((res) => res.json())
      .then((data: Product[]) => {
          ...
          .map((p) => ({
            ...
            mainImage: p.mainImage
              ? p.mainImage.startsWith("http")
                ? p.mainImage
                : `https://chupstore.onrender.com${p.mainImage}`
              : undefined,
          }))
          .sort((a, b) => b.id - a.id);
        ...
        const uniqueCategories = Array.from(new Set(updatedData.map((p) => p.category)));
      ...
      <div style={{ display: "flex", gap: "30px", flexWrap: windowWidth <= 1024 ? "wrap" : "nowrap" }}>
        ...
        {windowWidth > 1024 && (
          ...
          >
            ...
            >
              <h3 style={{ marginBottom: "12px", fontSize: "16px", fontWeight: "700", color: "#111" }}>
                კატეგორიები
              </h3>
              ...
              <Link key={p.id} href={`/products/${p.id}`} style={{ textDecoration: "none", color: "inherit" }}>
                ...
                >
                  <div>
                    ...
                    <p style={{ marginBottom: "10px", color: "#555" }}>{p.description.slice(0, 20) + "..."}</p>
                  </div>
