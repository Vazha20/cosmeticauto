...
export default function ProductDetailsPage() {
  ...
  useEffect(() => {
    if (!id) return;
    fetch(`https://chupstore.onrender.com/api/products/${id}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleDelete = async () => {
    ...
    Modal.confirm({
      ...
      async onOk() {
        try {
          const res = await fetch(`https://chupstore.onrender.com/api/products/${id}`, { method: "DELETE", headers: { "Content-Type": "application/json" } });
  ...
  const handleUpdate = async () => {
    if (!id) return;
    try {
      const res = await fetch(`https://chupstore.onrender.com/api/products/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editedProduct) });
      if (res.ok) {
          ...
          <Card>
              ...
              <Carousel ref={setCarouselRef} dots={false} autoplay={false}>
                ...
                {product.mainImage && (
                  <div className="carousel-slide">
                    <img src={product.mainImage.startsWith("http") ? product.mainImage : `https://chupstore.onrender.com${product.mainImage}`} alt={product.name} className="carousel-img" />
                  </div>
                )}
                {/* დანარჩენი ფოტოები */}
                {product.images?.map((img, idx) => (
                  <div key={idx} className="carousel-slide">
                    <img src={img.startsWith("http") ? img : `https://chupstore.onrender.com${img}`} alt={product.name} className="carousel-img" />
                  </div>
                ))}
              </Carousel>
            ...
            <div style={{ display: "flex", justifyContent: "center", marginTop: 16, gap: 10 }}>
              ...
              {allImages.map((img, idx) => (
                <Image
                  key={idx}
                  src={img.startsWith("http") ? img : `https://chupstore.onrender.com${img}`}
                  alt={`thumbnail-${idx}`}
                  width={70}
                  height={70}
